...
- 详情卡片采用紧凑的“市场 / 账户”底部分页，可显示 U 本位总资产、总未实现盈亏，以及币本位逐币种资产和非零持仓。
...
请为 CryptoTray 单独创建仅具备账户读取权限的 API Key，不要启用交易或提现权限。右键菜单中的“删除 API 凭据”会清除持久凭据并停止账户同步；环境变量只在当前 PowerShell 会话及其启动的程序中有效。
