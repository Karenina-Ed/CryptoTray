...
- 通过 Binance USD-M WebSocket 实时获取 U 本位永续合约的 UTC 日线行情，任务栏固定展示 BTCUSDT 和 ETHUSDT。
- 将紧凑的单行行情窗口嵌入 Windows 任务栏，横向显示 BTC、ETH 价格和 UTC 涨跌幅。
- 使用 OKX 风格的透明背景、高对比文字、清晰数字字体和克制的涨跌配色。
- 任务栏组件的完整矩形均可单击；左键单击后显示 OKX 风格详情卡片，点击卡片外部自动关闭。
- 市场页采用交易所式自选行情表，可搜索当前可交易的 U 本位永续合约，添加或移除自选，并显示最新价、UTC 自然日涨跌幅和成交量；自选列表会自动保存。
- 详情卡片采用紧凑的“市场 / 账户”底部分页，右上角可显示或隐藏总资产；现货、U 本位、币本位、期权及 Simple Earn 活期和定期权益统一估值，并显示账户明细与非零仓位。
- 账户页通过下拉框切换 USDT、BTC、ETH 或 CNY 计价，首次启动默认 USDT，选择结果会自动保存；BTC、ETH 使用永续行情换算，CNY 使用欧洲中央银行每日参考汇率近似换算。
